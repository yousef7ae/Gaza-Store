<div class="row no-gutters align-items-center login">
    ...
    <div class="col-sm-8  h-100 ">
        <div class="mx-auto px-3 py-1 max-w-650p py-3">
            <h3 class="text-secondary">{{$page ? $page->title_lang : ""}} <span class="badge bg-success text-white">{{($role = \Spatie\Permission\Models\Role::where('id',$user['role_id'])->first()) ? $role->name:__("Undefined")}}</span></h3>
            <p class="mb-0">{{$page ? $page->description_lang : ""}}</p>

            @foreach(\Spatie\Permission\Models\Role::whereNotIn('name',['Admin','Hilfer'])->orderBy('id','DESC')->get() as $role)
                <button wire:click="changeRole({{$role->id}})"  class="btn {{$role->id == $user['role_id'] ? 'btn-success':'btn-info'}}">{{$role->name}}</button>
            @endforeach
            <form wire:submit.prevent="register" method="post">
                ...
                <div class="row">
                    ...
                    <div class="form-group mb-1 col-6">
                        <label class="text-secondary" for="Country">{{ __('Select Country') }}</label>
                    ...
                    <div class="form-group mb-1 col-6">
                        <label class="text-secondary" for="Street">{{__("Address")}}</label>
                    ...
                    <div class="form-group mb-1 col-6">
                        <label class="text-secondary" for="Mobile-Number">{{__("Mobile Number")}}</label>
                        <div class="row shadow-sm rounded no-gutters overflow-hidden">
                            <select wire:model="user.country_id" disabled
                    ...
                    <div class="form-group mb-1 col-6">
                        <label class="text-secondary" for="Password">{{__("Password")}}</label>
                    ...
                    <div class="form-group mb-1 col-12">
                        <label class="text-secondary" for="Confirm-Password">{{__("Confirm Password")}}</label>
                    ...
                    <div class="form-group mb-1 col-12">
                        <div class="form-inline pl-sm-0 pl-4 my-2">
                            <input type="checkbox" wire:model="user.check" class="form-check-input @error('user.check') is-invalid @enderror" id="exampleCheck1">
                            <label class="form-check-label" for="exampleCheck1">{{__("Approve the")}}</label>
                            <a class="text-secondary font-weight-bold mx-2" data-toggle="modal" data-target="#conditions"
