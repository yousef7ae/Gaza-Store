                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        @if (auth()->user()->can('categories create'))
                            <a class="btn btn-primary float-end" wire:click.prevent="CreateCategory"
                                data-bs-toggle="modal" data-bs-target="#CreateCategory" data-bs-original-title=""
                                title=""> {{ __('Create Category') }}</a>
                        @endif
                    </div>
                </div>
            </div>
                                ...
                                <div class="input-group mb-3 " style="justify-content: center">
                                    ...
                                    <div class="input-group-append ">
                                        <button wire:loading.attr="disabled" class="btn btn-block btn-primary btn-sm"
                                            type="submit"><i wire:loading class="fas fa-sync fa-spin"></i>
                                            {{ __('Search') }}
                                        </button>
                                    </div>
                                </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <img width="50" class="rounded-circle img-thumbnail"
                                                            src="{{ $category->image ? $category->image : url('dashboard/images/image1.png') }}"
                                                    ...
                                                    <td>
                                                        @if ($category->status == 1)
                                                            <span
                                                                class="btn btn-success btn-xs">{{ \App\Models\Category::statusList($category->status) }}</span>
                                                        @elseif($category->status == 0)
                                                            <span
                                                                class="btn btn-danger btn-xs">{{ \App\Models\Category::statusList($category->status) }}</span>
                                                        @else
                                                            <span
                                                                class="btn btn-primary btn-xs">{!! \App\Models\Category::statusList($category->status) !!}</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        @if (auth()->user()->can('categories edit') or
                                                            ...
                                                            @if (auth()->user()->hasRole('Admin'))
                                                                <a class="btn btn-xs btn-success  {{ $category->status == 1 ? 'disabled' : '' }}"
                                                                    href="#"
                                                                    wire:click.prevent="Status({{ $category->id }})"
                                                                    data-bs-toggle="modal" data-bs-target="#activeModal"
                                                                    title="{{ __('Active') }}"><i
                                                                        class="fa fa-check"></i>
                                                                </a>

                                                                <a class="btn btn-xs btn-danger {{ $category->status == 0 ? 'disabled' : '' }}"
                                                                    href="#"
                                                                    wire:click.prevent="Status({{ $category->id }})"
