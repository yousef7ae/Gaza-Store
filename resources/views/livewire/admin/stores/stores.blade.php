                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        @if(auth()->user()->can('stores create'))
                            <a class="btn btn-primary float-end" data-bs-toggle="modal" data-bs-target="#CreateStore"
                               data-bs-original-title="" title=""> {{__('Create Store')}}</a>
                        @endif
                    </div>
                </div>
            </div>
                            ...
                            <form class="col-md-12" wire:submit.prevent="search">
                                <div class="input-group mb-3 " style="justify-content: center">
                                    ...
                                    <div class="mb-2">
                                        <input type="text" class="form-control form-control-sm"
                                               style="border-radius: .1875rem !important; margin-left: 10px !important"
                                               placeholder="{{__("Phone")}}" wire:model.defer="phone">
                                    </div>
                                    ...
                                    <div class="mb-2" style="width: 170px; ">
                                        <select wire:model.defer="status"
                                            ...
                                            @foreach(\App\Models\Store::statusList(false) as $key => $value)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <img width="50" class="rounded-circle img-thumbnail"
                                                         src="{{ $store->image ? $store->image : url('dashboard/images/image1.png')}}"
                                                ...
                                                <td>
                                                    @if($store->status == 1  )
                                                        <span
                                                            class="btn btn-success btn-xs">{{ \App\Models\Store::statusList($store->status)}}</span>
                                                    @elseif( $store->status == 0)
                                                        <span
                                                            class="btn btn-danger btn-xs">{{ \App\Models\Store::statusList($store->status)}}</span>
                                                    @else
                                                        <span
                                                            class="btn btn-primary btn-xs">{!! \App\Models\Store::statusList($store->status) !!}</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if(auth()->user()->can('stores edit'))

                                                        <a class="btn btn-xs btn-success  {{$store->status == 1 ? 'disabled' : ''}}"
                                                           href="#"
                                                           wire:click.prevent="Status({{$store->id}})"
                                                           data-bs-toggle="modal" data-bs-target="#activeModal"
                                                           title="{{__("Active")}}"><i class="fa fa-check"></i>
                                                        </a>

                                                        <a class="btn btn-xs btn-danger {{$store->status == 0 ? 'disabled' : ''}}"
                                                           href="#"
                                                           wire:click.prevent="Status({{$store->id}})"
                                                        ...
                                                        <a class="btn btn-xs btn-primary"
                                                           href="#" data-bs-toggle="modal" data-bs-target="#EditStore"
                                                    ...
                                                    @if(auth()->user()->can('stores delete'))
                                                        <a class="btn btn-xs btn-danger" href="#"
                                                           wire:click.prevent="deleteId({{$store->id}})"
...
@if(auth()->user()->can('stores create'))
    <!--  Modal CreateStore -->
        <div wire:ignore.self class="modal fade " id="CreateStore" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content text-center">
                    <div class="modal-header text-center">
                        <h5 class="modal-title text-center" id="exampleModalLongTitle">{{ __('Add New Store') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true close-btn"></span>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        @livewire('admin.stores.stores-create')
                    </div>
                </div>
            </div>
        </div>
...
@if(auth()->user()->can('stores edit'))
    <!--  Modal EditStore -->
        <div wire:ignore.self class="modal fade " id="EditStore" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content text-center">
                    <div class="modal-header text-center">
                        <h5 class="modal-title text-center" id="exampleModalLongTitle">{{ __('Store') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true close-btn"></span>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        @if($store_id)
                            @livewire('admin.stores.stores-edit',[$store_id])
                        @endif
                    </div>
                </div>
            </div>
