                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{__('Carts')}}</li>
                        </ol>
                    </div>
                </div>
            </div>
                            ...
                            <form class="col-md-12" wire:submit.prevent="search">
                                <div class="input-group mb-3 " style="justify-content: center">
                                    ...
                                    <div class="mb-2">
                                        <input type="text" class="form-control form-control-sm"
                                               style="border-radius: .1875rem !important; margin-left: 10px !important"
                                               placeholder="{{__("Qty")}}" wire:model.defer="qty">
                                    </div>
                                ...
                                <div class="table-responsive-sm">
                                    <table class="table color-bordered-table info-table  info-bordered-table">
                                        ...
                                        @foreach($carts as  $cart)
                                            <tr>
                                                <td>{{$cart->id}}</td>
                                                <td>{{ $cart->user ? $cart->user->name : 'Customer'}}</td>
                                                <td>
                                                    <a href="{{route('admin.carts.show',$cart->id)}}">{{$cart->price_string}}</a>
                                                </td>
                                                <td><a href="{{route('admin.carts.show',$cart->id)}}">{{$cart->qty}}</a>
                                                </td>
                                                <td>
                                                    <a href="{{route('admin.carts.show',$cart->id)}}">{{$cart->total}}</a>
                                                </td>

                                                <td>
                                                    @if(auth()->user()->can('carts show'))
                                                    ...
                                                    @if(auth()->user()->can('carts delete'))
                                                        <a class="btn btn-xs btn-danger" href="#"
                                                           wire:click.prevent="deleteId({{$cart->id}})"
