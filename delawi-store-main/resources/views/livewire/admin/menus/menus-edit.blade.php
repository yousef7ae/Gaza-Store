        ...
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            @foreach(config('app.locales') as $locale => $language)
                <button class="nav-link {{$locale == app()->getLocale() ? 'active' : ''}}"
                        id="nav-lang-edit-{{$locale}}-tab"
                        data-bs-toggle="tab" data-bs-target="#nav-lang-edit-{{$locale}}" type="button" role="tab"
                        aria-controls="nav-lang-edit-{{$locale}}" aria-selected="true">
                    {{$locale}}</button>
            @endforeach
        </div>
    </nav>
    ...
    <div class="tab-content" id="nav-tabContent-edit">
        @foreach(config('app.locales') as $locale => $locale)
            <div class="tab-pane fade {{$locale == app()->getLocale() ? 'show active' : ''}}"
                 id="nav-lang-edit-{{$locale}}"
                 role="tabpanel" aria-labelledby="nav-lang-edit-{{$locale}}-tab">
