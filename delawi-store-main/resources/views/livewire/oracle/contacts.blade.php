            ...
            <div class="row">
                <div class="col-md-6">
                    <form method="post" wire:submit.prevent="store" class="mb-3 px-md-0 px-3">
                        ...
                        <div class="form-group mb-4">
                            <label class="text-danger" for="Title">{{__("Title")}}</label>
                        ...
                        <div class="form-group mb-4">
                            <label class="text-danger" for="textarea-d">{{__("Message")}}</label>
                        ...
                        <div class="col-6">
                            <a href="tel:{{($setting = \App\Models\Setting::where('name',"phone")->first()) ? $setting->value : __("#")}}"
                               class="nav-item nav-link px-2 text-dark d-flex align-items-center "><i
                                    class="fas fa-phone-volume fs-25p pr-3 text-danger"></i> {{($setting = \App\Models\Setting::where('name',"phone")->first()) ? $setting->value : __("Empty")}}
                            </a>
                            <a href="#" class="nav-item nav-link px-2 text-dark d-flex align-items-center "><i
                                    class="fas fa-map-marker-alt fs-25p pr-3 text-danger"></i> {{($setting = \App\Models\Setting::where('name',"address")->first()) ? $setting->value : __("Empty")}}
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="mailto:{{($setting = \App\Models\Setting::where('name',"email")->first()) ? $setting->value : __("#")}}"
                               class="nav-item nav-link px-2 text-dark d-flex align-items-center "><i
                                    class="far fa-envelope fs-25p pr-3 text-danger"></i> {{($setting = \App\Models\Setting::where('name',"email")->first()) ? $setting->value : __("Empty")}}
                            </a>
                            <a href="{{($setting = \App\Models\Setting::where('name',"url_whatsapp")->first()) ? $setting->value : '#'}}"
                               target="_blank" class="nav-item nav-link px-2 text-dark d-flex align-items-center "><i
                                    class="fab fa-whatsapp fs-25p pr-3 text-danger"></i> {{($setting = \App\Models\Setting::where('name',"mobile")->first()) ? $setting->value : __("Empty") }}
                            </a>
                        </div>
                        <div class="col-12">
                            <p>
